...
让我们查看 XML 格式的 Toolbox 数据。`ElementTree`的`write()`方法需要一个文件对象。我们通常使用 Python 内置的`open()`函数创建。为了屏幕上显示输出，我们可以使用一个特殊的预定义的文件对象称为`stdout` [![[1]](Images/346344c2e5a627acfdddf948fb69cb1d.jpg)](./ch11.html#sys-stdout) （标准输出），在 Python 的`sys`模块中定义的。
