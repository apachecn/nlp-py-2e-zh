...
更一般的，我们可以在(i, j)输入 *A*，如果有一个产生式 *A* → *B* *C*，并且我们在(i, k)中找到非终结符 *B*，在(k, j)中找到非终结符 *C*。[4.4](./ch08.html#code-wfst)中的程序使用此规则完成 WFST。通过调用函数`complete_wfst()`时设置 `trace`为`True`，我们看到了显示 WFST 正在被创建的跟踪输出：
