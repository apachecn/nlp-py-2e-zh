...
在[1.7](./ch06.html#code-consecutive-pos-tagger)中演示了这一策略。首先，我们必须扩展我们的特征提取函数使其具有参数`history`，它提供一个我们到目前为止已经为句子预测的标记的列表[![[1]](Images/78bc6ca8410394dcf6910484bc97e2b6.jpg)](./ch06.html#consec-pos-tag-features)。`history`中的每个标记对应`sentence`中的一个词。但是请注意，`history`将只包含我们已经归类的词的标记，也就是目标词左侧的词。因此，虽然是有可能查看目标词右边的词的某些特征，但查看那些词的标记是不可能的（因为我们还未产生它们）。
