...
如果你习惯了使用特定的本地编码字符，你可能希望能够在一个 Python 文件中使用你的字符串输入及编辑的标准方法。为了做到这一点，你需要在你的文件的第一行或第二行中包含字符串：`'# -*- coding: &lt;coding&gt; -*-'`。请注意 *&lt;coding&gt;* 必须是像`'latin-1'`, `'big5'`或`'utf-8'`这样的字符串 （见 [3.4](./ch03.html#fig-polish-utf8)）。
