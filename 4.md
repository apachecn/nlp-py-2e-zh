...
我们首先定义函数的两个参数，`msg`和`num` [❶](./ch04.html#fun-def)。然后调用函数，并传递给它两个参数，`monty`和`3` [❷](./ch04.html#fun-call)；这些参数填补了参数提供的“占位符”，为函数体中出现的`msg`和`num`提供值。
