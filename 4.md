...
```py
>>> foo = 'Monty'
>>> bar = foo ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
>>> foo = 'Python' ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
>>> bar
'Monty'
```

这个结果与预期的完全一样。当我们在上面的代码中写`bar = foo`时[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#assignment1)，`foo`的值（字符串`'Monty'`）被赋值给`bar`。也就是说，`bar`是`foo`的一个副本，所以当我们在第[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#assignment2)行用一个新的字符串`'Python'`覆盖`foo`时，`bar`的值不会受到影响。

然而，赋值语句并不总是以这种方式复制副本。赋值总是一个表达式的值的复制，但值并不总是你可能希望的那样。特别是结构化对象的“值”，例如一个列表，实际上是一个对象的引用。在下面的例子中，[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#assignment3)将`foo`的引用分配给新的变量`bar`。现在，当我们在[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#assignment4)行修改`foo`内的东西，我们可以看到`bar`的内容也已改变。

```py
>>> foo = ['Monty', 'Python']
>>> bar = foo ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
>>> foo[1] = 'Bodkin' ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
>>> bar
['Monty', 'Bodkin']
```
...
`bar = foo`[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#assignment3)行并不会复制变量的内容，只有它的“引用对象”。要了解这里发生了什么事，我们需要知道列表是如何存储在计算机内存的。在[4.1](./ch04.html#fig-array-memory)中，我们看到一个列表`foo`是对存储在位置 3133 处的一个对象的引用（它自身是一个指针序列，其中的指针指向其它保存字符串的位置）。当我们赋值`bar = foo`时，仅仅是 3133 位置处的引用被复制。这种行为延伸到语言的其他方面，如参数传递（[4.4](./ch04.html#sec-functions)）。
...
因为表达式中`if`子句条件满足，Python 就不会求值`elif`子句，所以我们永远不会得到输出`2`。相反，如果我们用一个`if`替换`elif`，那么我们将会输出`1`和`2`。所以`elif`子句比单独的`if`子句潜在地给我们更多信息；当它被判定为真时，告诉我们不仅条件满足而且前面的`if`子句条件 _ 不 _ 满足。
...
到目前为止，我们已经看到了两种序列对象：字符串和列表。另一种序列被称为元组。元组由逗号操作符[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#create-tuple)构造，而且通常使用括号括起来。实际上，我们已经在前面的章节中看到过它们，它们有时也被称为“配对”，因为总是有两名成员。然而，元组可以有任何数目的成员。与列表和字符串一样，元组可以被索引[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#index-tuple)和切片[![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)](./ch04.html#slice-tuple)，并有长度[![[4]](Images/ca21bcde8ab16a341929b7fb9ccb0a0e.jpg)](./ch04.html#length-tuple)。

```py
>>> t = 'walk', 'fem', 3 ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
>>> t
('walk', 'fem', 3)
>>> t[0] ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
'walk'
>>> t[1:] ![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)
('fem', 3)
>>> len(t) ![[4]](Images/ca21bcde8ab16a341929b7fb9ccb0a0e.jpg)
3
```
...
对于一些 NLP 任务，有必要将一个序列分割成两个或两个以上的部分。例如，我们可能需要用 90％的数据来“训练”一个系统，剩余 10％进行测试。要做到这一点，我们指定想要分割数据的位置[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#cut-location)，然后在这个位置分割序列[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#cut-sequence)。

```py
>>> text = nltk.corpus.nps_chat.words()
>>> cut = int(0.9 * len(text)) ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
>>> training_data, test_data = text[:cut], text[cut:] ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
>>> text == training_data + test_data ![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)
True
>>> len(training_data) / len(test_data) ![[4]](Images/ca21bcde8ab16a341929b7fb9ccb0a0e.jpg)
9.0
```

我们可以验证在此过程中的原始数据没有丢失，也没有重复[![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)](./ch04.html#cut-preserve)。我们也可以验证两块大小的比例是我们预期的[![[4]](Images/ca21bcde8ab16a341929b7fb9ccb0a0e.jpg)](./ch04.html#cut-ratio)。
...
```py
>>> words = 'I turned off the spectroroute'.split() ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
>>> wordlens = [(len(word), word) for word in words] ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
>>> wordlens.sort() ![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)
>>> ' '.join(w for (_, w) in wordlens) ![[4]](Images/ca21bcde8ab16a341929b7fb9ccb0a0e.jpg)
'I off the turned spectroroute'
```

上述代码段中每一行都包含一个显著的特征。一个简单的字符串实际上是一个其上定义了方法如`split()` [![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#string-object)的对象。我们使用列表推导建立一个元组的列表[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#tuple-comprehension)，其中每个元组由一个数字（词长）和这个词组成，例如`(3, 'the')`。我们使用`sort()`方法[![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)](./ch04.html#sort-method)就地排序列表。最后，丢弃长度信息，并将这些词连接回一个字符串[![[4]](Images/ca21bcde8ab16a341929b7fb9ccb0a0e.jpg)](./ch04.html#discard-length)。（下划线[![[4]](Images/ca21bcde8ab16a341929b7fb9ccb0a0e.jpg)](./ch04.html#discard-length)只是一个普通的 Python 变量，我们约定可以用下划线表示我们不会使用其值的变量。）
...
假设我们现在想要进一步处理这些词。我们可以将上面的表达式插入到一些其他函数的调用中[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#max-comprehension)，Python 允许我们省略方括号[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#max-generator)。

```py
>>> max([w.lower() for w in word_tokenize(text)]) ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
'word'
>>> max(w.lower() for w in word_tokenize(text)) ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
'word'
```

第二行使用了生成器表达式。这不仅仅是标记方便：在许多语言处理的案例中，生成器表达式会更高效。在[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#max-comprehension)中，列表对象的存储空间必须在 max()的值被计算之前分配。如果文本非常大的，这将会很慢。在[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#max-generator)中，数据流向调用它的函数。由于调用的函数只是简单的要找最大值——按字典顺序排在最后的词——它可以处理数据流，而无需存储迄今为止的最大值以外的任何值。
...
我们首先定义函数的两个参数，`msg`和`num` [![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#fun-def)。然后调用函数，并传递给它两个参数，`monty`和`3` [![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#fun-call)；这些参数填补了参数提供的“占位符”，为函数体中出现的`msg`和`num`提供值。
...
下面是一个更复杂的生成器的例子，产生一个词列表的所有排列。为了强制`permutations()`函数产生所有它的输出，我们将它包装在`list()`调用中[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#listperm)。
...
...
>>> list(permutations(['police', 'fish', 'buffalo'])) ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
...
...             result.append(word)
...     return result
>>> find_words(['omg', 'teh', 'lolcat', 'sitted', 'on', 'teh', 'mat'], 3) ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
['omg', 'teh', 'teh', 'mat']
>>> find_words(['omg', 'teh', 'lolcat', 'sitted', 'on', 'teh', 'mat'], 2, ['ur']) ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
['ur', 'on']
>>> find_words(['omg', 'teh', 'lolcat', 'sitted', 'on', 'teh', 'mat'], 3) ![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)
['omg', 'teh', 'teh', 'mat', 'omg', 'teh', 'teh', 'mat']
```

我们第一次调用`find_words()`[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#find-words-1)，我们得到所有预期的三个字母的词。第二次，我们为 result 指定一个初始值，一个单元素列表`['ur']`，如预期，结果中有这个词连同我们的文本中的其他双字母的词。现在，我们再次使用[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#find-words-1)中相同的参数调用`find_words()`[![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)](./ch04.html#find-words-3)，但我们得到了不同的结果！我们每次不使用第三个参数调用`find_words()`，结果都只会延长前次调用的结果，而不是以在函数定义中指定的空链表 result 开始。程序的行为并不如预期，因为我们错误地认为在函数被调用时会创建默认值。然而，它只创建了一次，在 Python 解释器加载这个函数时。这一个列表对象会被使用，只要没有给函数提供明确的值。
...
我们也可以设计一种这个问题的迭代解决方案处理层的层次结构。第一层是同义词集本身[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#first-layer)，然后是同义词集所有的下位词，之后是所有下位词的下位词。每次循环通过查找上一层的所有下位词计算下一层[![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)](./ch04.html#update-layer)。它也保存了到目前为止遇到的同义词集的总数[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#update-total)。

```py
>>> def size2(s):
...     layer = [s] ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
...     total = 0
...     while layer:
...         total += len(layer) ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
...         layer = [h for c in layer for h in c.hyponyms()] ![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)
...     return total
```
...
我们可以使用`timeit`模块测试这种说法。`Timer`类有两个参数：一个是多次执行的语句，一个是只在开始执行一次的设置代码。我们将分别使用一个整数的列表[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#vocab-list)和一个整数的集合[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#vocab-set)模拟 10 万个项目的词汇表。测试语句将产生一个随机项，它有 50％的机会在词汇表中[![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)](./ch04.html#vocab-statement)。

```py
>>> from timeit import Timer
>>> vocab_size = 100000
>>> setup_list = "import random; vocab = range(%d)" % vocab_size ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
>>> setup_set = "import random; vocab = set(range(%d))" % vocab_size ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
>>> statement = "random.randint(0, %d) in vocab" % (vocab_size * 2) ![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)
...
也可以动态的产生这些数据的可视化图形。例如，一个使用表单输入的网页可以允许访问者指定搜索参数，提交表单，看到一个动态生成的可视化图形。要做到这一点，我们必须为`matplotlib`指定`Agg`后台，它是一个产生栅格（像素）图像的库[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#agg-backend)。下一步，我们像以前一样使用相同的 Matplotlib 方法，但不是用`pyplot.show()`显示结果在图形终端，而是使用`pyplot.savefig()`把它保存到一个文件[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#pyplot-savefig)。我们指定文件名，然后输出一些 HTML 标记指示网页浏览器来加载该文件。

```py
>>> from matplotlib import use, pyplot
>>> use('Agg') ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
>>> pyplot.savefig('modals.png') ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
...
def traverse(graph, start, node):
    ...
    for child in node.hyponyms():
        graph.add_edge(node.name, child.name) ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
        traverse(graph, start, child) ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)

def hyponym_graph(start):
    G = nx.Graph() ![[3]](Images/496754d8cdb6262f8f72e1f066bab359.jpg)
    G.depth = {}
...
我们可以使用 Python 的 CSV 库读写这种格式存储的文件。例如，我们可以打开一个叫做`lexicon.csv`的 CSV 文件[![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)](./ch04.html#open-csv)，并遍历它的行[![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)](./ch04.html#iterate-csv)：

```py
>>> import csv
>>> input_file = open("lexicon.csv", "rb") ![[1]](Images/ffa808c97c7034af1bc2806ed7224203.jpg)
>>> for row in csv.reader(input_file): ![[2]](Images/aa68e0e8f4d58caa31e5542dabe4ddc2.jpg)
...     print(row)
